...
const DataChart = ({ jsonString, title }) => {
    ...
    // --- Feature Filtering (Excludes known index/unnamed columns) ---
    ...
    if (datetimeIndex === -1 || featureColumns.length === 0) {
        // If 'datetime' exists, but only 'Actual_Value' and 'Predicted_Value' exist (as expected for regression detailed data),
        // we override the filtering to include them, if they were removed by accident.
        const regressionFeatures = columns.filter(col => col === 'Actual_Value' || col === 'Predicted_Value');
        if (datetimeIndex !== -1 && regressionFeatures.length > 0) {
             featureColumns.push(...regressionFeatures.filter(f => !featureColumns.includes(f)));
        } else if (datetimeIndex === -1 || featureColumns.length === 0) {
    ...
    return (
        // FIX 1: Changed background to white and updated border/shadow
            ...
            <ResponsiveContainer width="100%" height={400}>
                ...
                >
                    ...
                    <YAxis 
                        ...
                        type="number"
                        domain={['auto', 'auto']} // Changed from fixed domain for regression results
                    ...
                    <Brush 
                        ...
                        height={30}
                        stroke="#8884d8"
                        // FIX 8: Changed brush fill to light gray and added performance fix
                        fill="#F3F4F6"
                        interval={5} // PERFORMANCE FIX
                    />
